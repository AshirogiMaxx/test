...
void printDay(int day)
{
  switch(day)
  {
    ...
    default: display.print("--- ");break; //oled dashes if error - avoids scrambling display
  } 
}
...
void setup()
{
  ...
  display.init();                      // Initialze SSD1306 display display
  display.clear();              // Clear screen
  //display.drawBitmap(ACROBOT, 1024);   // 1024 pixels for logo
  //display.setCursor(0,1);              // Set cursor position
...
void loop()
{
  ...
  display.setBrightness((unsigned char)brightness);
  //display.setPageMode();
  display.setPixel(7,13);
  display.print(brightness);       
  delay(500);
  if(brightness == 255)
  {
    display.setPixel(7,13);           // Display the brighness value starting from 7th Row, 13th Column
    display.print("   ");          // Clear 13,14 and 15th Columns. Numbers left aligned.
  ...
  if (mode <= 7){
    ...
    display.print(tempF);
    //display.drawCircle(124, 8, 2, WHITE);

    display.setPixel(0,0);

    printDay(20);
    display.setPixel(16,0);
    printMonth(9);        
    }
   }
}
